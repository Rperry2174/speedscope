...
import {BuildPerfReportInput, BrowserBenchmarkReport, EngineRunOptions, ParityReport} from './types'
...
function buildPrompt(role: SpecialistRole, engineRuns: EngineRunOptions[]) {
  ...
  return [
    ...
    ...shardLines,
    '',
    'Current engine runs under comparison:',
    ...engineRuns.map(engine => `  - ${JSON.stringify(engine)}`),
    '',
    'Constraints:',
...
async function runSpecialist(
  ...
  try {
    ...
    const run = await agent.send(buildPrompt(specialist.role, engineRuns))
...
async function main() {
  ...
  const experimentFlags: EngineRunOptions[] = [
    {
      name: 'legacy',
      importEngine: 'legacy',
      compareImport: false,
