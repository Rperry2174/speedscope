...
const EXPERIMENTS: ExperimentDefinition[] = [
  ...
  {
    id: 'rust-firefox-import',
    label: 'Rust Firefox import',
    description:
      'Experimental path that parses Firefox profiles through the Rust/WASM payload extractor with a TypeScript fallback.',
    flags: {
      deferDemangle: false,
      optimizedForEachCall: false,
      rustFuzzyFind: false,
  ...
  {
    id: 'rust-v8-cpu-formatter',
    label: 'Rust V8 CPU formatter',
    description:
      'Experimental path that routes old V8 CPU profile normalization through a Rust/WASM implementation with a TS fallback.',
    flags: {
      deferDemangle: false,
      optimizedForEachCall: false,
      rustFuzzyFind: false,
